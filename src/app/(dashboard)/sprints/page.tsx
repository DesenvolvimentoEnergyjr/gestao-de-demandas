...
const getDynamicSprintStatus = (sprint: Sprint, sprintDemands: Demand[]) => {
  ...
  const startDate = (sprint.startDate as unknown as { toDate?: () => Date })
    ?.toDate
    ? (sprint.startDate as unknown as { toDate: () => Date }).toDate()
    : new Date(sprint.startDate as string | Date);
...
export default function SprintsPage() {
  ...
  const filteredSprints = useMemo(() => {
    const visibleDemandsIds = new Set(
      demands
        .filter((d) => isDemandVisibleToUser(d, user, users))
        .map((d) => d.id),
    );
    ...
    return sprints.filter((s) => {
      ...
      if (user?.role === "assessor" && s.demandIds && s.demandIds.length > 0) {
        const hasVisibleDemand = s.demandIds.some((id) =>
          visibleDemandsIds.has(id),
        );
      ...
      if (activeFilter === "backlog")
        return dynamicStatus === "planned" || dynamicStatus === "paused";
  ...
  const renderSprintCard = (sprint: Sprint) => {
    ...
    const projectTypeId =
      sprint.tags?.find((t) => t === "Interno" || t === "Externo") ||
      tenantConfig.projectTypes[0].id;
    const projectConfig =
      tenantConfig.projectTypes.find((p) => p.id === projectTypeId) ||
      tenantConfig.projectTypes[0];
    const theme = projectConfig.theme;

    const totalHours = sprintDemands.reduce(
    ...
    if (dynamicStatus === "active") {
      styles = {
        borderClass:
          theme === "secondary" ? "border-secondary" : "border-primary",
      ...
      styles = {
        borderClass:
          theme === "secondary"
            ...
            : "border-primary-dark",
        textClass:
          theme === "secondary" ? "text-secondary-dark" : "text-primary-dark",
        bgClass:
          theme === "secondary" ? "bg-secondary-dark/10" : "bg-primary-dark/10",
        label: "CONCLUÍDA",
      };
    ...
    return (
      <Card
        key={sprint.id}
        className={cn(
          "p-6 md:p-7 flex flex-col gap-5 border border-l-[6px] transition-all bg-[#111111]",
          styles.borderClass,
        )}
      >
        <div className="flex justify-between items-start">
          ...
          <div
            className={cn(
              "px-2.5 py-1 rounded-md text-[10px] font-black tracking-tighter",
              styles.bgClass,
              styles.textClass,
            )}
          >
        ...
        <div className="space-y-4 py-2">
          <div className="flex items-center gap-3 text-xs">
            ...
            <span className="font-medium text-zinc-300 line-clamp-2 leading-relaxed">
              {sprint.objective}
            </span>
          </div>
        ...
        {loading ? (
          <div className="border border-white/5 bg-white/[0.01] rounded-2xl md:rounded-[40px] p-4 md:p-8 mb-10">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-64 bg-white/[0.02] rounded-3xl animate-pulse border border-white/5"
                />
              ))}
            </div>
          </div>
          ...
          <div className="space-y-12 pb-10">
            {groupedSprints.gestaoAtual.length > 0 && (
              <div>
                <h2 className="text-xl md:text-2xl font-black text-white tracking-tight mb-6">
                  Gestão Atual
                </h2>
            ...
            {groupedSprints.pastGroups.length > 0 && (
              <div className="space-y-8">
                ...
                {groupedSprints.pastGroups.map((group) => (
                  <div
                    key={group.label}
                    className="border border-white/5 bg-white/[0.01] rounded-2xl md:rounded-[40px] p-4 md:p-8"
                  >
                    <h3 className="text-lg font-black text-secondary uppercase tracking-[0.2em] mb-6">
                      {group.label}
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          <div className="border border-white/5 bg-white/[0.01] rounded-2xl md:rounded-[40px] p-4 md:p-8 mb-10 flex flex-col items-center justify-center border-dashed py-20 text-center">
            ...
            <p className="text-zinc-600 font-medium text-sm">
              Nenhuma sprint encontrada para este filtro.
            </p>
          </div>
        )}
      </div>
