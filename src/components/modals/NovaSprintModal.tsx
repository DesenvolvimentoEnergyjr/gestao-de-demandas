...
import { X, ArrowRight, Calendar, Target, Hash } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const sprintData: Partial<Sprint> = {
        number: sprints.length + 1, // Auto-increment number
      ...
      addSprint({ id: sprintId, ...sprintData, createdAt: new Date() } as Sprint);
      closeNovaSprint();
      resetForm();
    } catch (error) {
      console.error(error);
      ...
      <div className="relative w-full max-w-xl bg-[#0f0f0f] border border-white/[0.05] rounded-[32px] shadow-[0_0_50px_-12px_rgba(0,0,0,0.5)] overflow-hidden animate-in fade-in zoom-in-95 duration-500">
        ...
        <div className="p-8 pb-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500 font-medium font-outfit uppercase tracking-tighter">Planejamento e Metas • Energy Júnior</p>
            </div>
          </div>
          <button
            onClick={closeNovaSprint}
        ...
        <form onSubmit={handleSubmit} className="p-8 pt-6 space-y-6">
          ...
          <div className="space-y-3">
            <label className="text-[10px] font-black text-secondary uppercase tracking-[0.2em] ml-1">Título do Ciclo</label>
            <Input
              required
          ...
          <div className="space-y-3">
            <label className="text-[10px] font-black text-secondary uppercase tracking-[0.2em] ml-1">Objetivo da Sprint</label>
            <div className="relative">
              ...
              />
              <Target className="absolute left-3.5 top-3.5 w-4 h-4 text-zinc-600" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              <label className="text-[10px] font-black text-secondary uppercase tracking-[0.2em] ml-1">Previsão Término</label>
              <div className="relative">
                <Input
                  required
                  type="date"
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-3">
              <label className="text-[10px] font-black text-secondary uppercase tracking-[0.2em] ml-1">Meta de Pontos</label>
              <div className="relative">
                <Input
                  type="number"
                  value={formData.totalPoints}
                  onChange={e => setFormData(prev => ({ ...prev, totalPoints: Number(e.target.value) }))}
                  className="bg-zinc-950 border-white/[0.03] h-12 text-sm rounded-xl px-4 font-bold"
                />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-bold text-zinc-600 uppercase">PTS</span>
              </div>
            </div>
            <div className="space-y-3">
              <label className="text-[10px] font-black text-secondary uppercase tracking-[0.2em] ml-1">Tipo de Sprint</label>
              <div className="flex p-1 bg-zinc-950 border border-white/[0.03] rounded-xl h-12">
                <button
                  ...
                  className={cn(
                    ...
                    formData.type === 'Interno' ? "bg-white text-black shadow-lg" : "text-zinc-600 hover:text-zinc-400"
                  )}
                >Interno</button>
                <button
                  type="button"
                  ...
                  className={cn(
                    "flex-1 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all",
                    formData.type === 'Externo' ? "bg-[#0baf4d] text-white shadow-lg shadow-secondary/10" : "text-zinc-600 hover:text-zinc-400"
                  )}
                >Externo</button>
              </div>
            </div>
